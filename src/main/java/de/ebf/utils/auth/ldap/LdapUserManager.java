...
//      }
    ...
    protected LdapUser getUserByAttribute(LDAPConnection connection, String attribute, String value, String context) throws LdapException {
        ...
        return getUserByFilter(connection, filter, context);
    }

    public List<LdapUser> getUsersByFilter(LDAPConnection connection, Filter filter, String context) throws LdapException {
        List<LdapUser> users = new ArrayList<>();
        try {
            Filter userFilter = Filter.createEqualityFilter(LdapUtil.ATTR_OBJECTCLASS, LdapUtil.OBJECTCLASS_USER);
            Filter searchFilter;
//            if (LdapConfig.getType().equals(LdapType.ActiveDirectory)){
//                //Filter out any users that are within the builtin container because these cannot be moved
...
//                searchFilter = Filter.createANDFilter(userFilter, filter, notBuiltInFilter);
//            } else {
...
//            }
    ...
    private LdapUser getUserByFilter(LDAPConnection connection, Filter filter, String context) throws LdapException {
